...
class BasePipe extends Pipe {
	...
	private void readObject(final java.io.ObjectInputStream in)
		...
		if (stemmer instanceof UEALite) {
			...
			LOGGER.debug("Got UEALite stemmer in loaded model: {}",
					wrappedStemmer);
		} else if (null != snowballLanguage) {
			wrappedStemmer = new Stemmer(snowballStemmerLanguage);
			LOGGER.debug(
					"Got snowball stemmer for language {} in loaded model",
					snowballStemmerLanguage);
		} else
			throw new IllegalStateException(
					"No stemmer was found in the stored model, this is invalid.");
	}

	public BasePipe(final Properties featureConfig) {
		super(new Alphabet(), new LabelAlphabet());
		this.featureConfig = featureConfig;
		UpperCaseStart = Pattern.compile("[" + UNICODE_UPPER + "][^"
				+ UNICODE_UPPER + "]*");
		...
		if (pluralFeature)
			customPluralSuffixes = fc.getStringArray(featureConfig,
					"customPluralSuffixes");
		...
		// stemmer
		snowballLanguage = fc.getStringArray(featureConfig,
				"SnowballStemmerLanguage");
	...
	public Instance pipe(final Instance carrier) {
		...
		final LabelSequence target = new LabelSequence(
		...
		try {
			...
			for (int i = 0; i < sentenceUnits.size(); i++) {
				...
				if (pluralFeature)
					if (customPluralSuffixes == null) {
						...
						for (final String suffix : customPluralSuffixes)
							if (unstemmedWords[i].equals(stemmedWords[i]
									+ suffix)) {
								token.setFeatureValue("PLURAL", 1.0);
								break;
							}
				...
				if (wcFeature) {
					...
					wc = wc.replaceAll("[^" + UNICODE_UPPER + UNICODE_LOWER
							+ "0-9]", "x");
				...
				if (bwcFeature) {
					...
					bwc = bwc.replaceAll("[^" + UNICODE_UPPER + UNICODE_LOWER
							+ "0-9]+", "x");
		...
		Stemmer(final String lang) {
			try {
				final Class<?> stemClass = Class
						.forName("org.tartarus.snowball.ext." + lang
								+ "Stemmer");
