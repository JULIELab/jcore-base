...
public class MSTParserAnnotator extends JCasAnnotator_ImplBase {
	...
	@ConfigurationParameter(
			name = PARAM_MAX_NUM_TOKENS,
			description = "The maximum number of tokens a sentence may have to be subject to parsing. If a sentence has more tokens, it will be skipped by the component. If no value is given, no restriction of the number of tokens is imposed.")
		...
		if (parameters_valid) {
			...
			while (sentenceIterator.hasNext()) {
				...
				while (tokenIterator.hasNext()) {
					...
					if (token.getPosTag() == null || token.getPosTag().size() == 0 || token.getPosTag(0) == null) {
						...
						throw new IllegalStateException(
								"The parser expects that each token has (at least) one part of speech tag at index 0 of the PosTag feature array. However, the token \"" + token
										.getCoveredText()
										+ "\", offsets "
										+ token.getBegin()
										+ "-"
				...
				if (maxNumTokens != null && i > maxNumTokens) {
					LOGGER.warn("Skipping sentence with > " + maxNumTokens + " tokens: " + casSentence.getCoveredText());
					break;
				}
				...
				// }
					...
					String docId = getDocId(jcas);
					LOGGER.warn(
							"OutOfMemory error occured when parsing the sentence \"{}\" of document \"{}\". " + "This does not mean necessarely your application has too less allocated memory. "
				...
				try {
					...
					LOGGER.error("Sentence could not be parsed and will not have syntactic annotations in the CAS: " + casSentence
							.getCoveredText());
				}
			} // of while
		} else {
	...
	private void setProjective(DependencyRelation depRelation, DependencyParser mstParser) {
		...
		if (decodeType.equals(PROJECTIVE)) {
			...
			LOGGER.error("setProjective: decode mode is invalid. Setting projective to the default value " + defaultProjective);
