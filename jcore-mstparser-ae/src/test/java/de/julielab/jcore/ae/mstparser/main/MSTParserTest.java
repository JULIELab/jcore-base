...
public class MSTParserTest extends TestCase {
	...
	public void testCAS() throws Exception{
		String[] heads = new String[]{"have","Migrants","drown","coast","off","40",
				"40","migrants","have","have","drowned","Sea","Sea","in","drowned",
				"coast","coast","off","coast","of","drowned","drowned","officials",
				"have","corpses","were","were","found","washed","found","shore","on",
				"shore","city","near","city","of","were","people","were","were",
				"rescued","rescued","s","s","s","officials","agency","agency","s",
				"were","say","boat","say","say","was","overloaded","overloaded","60",
				"60","people","carrying","overloaded","sank","overloaded","sank","on",
				"Saturday", //actually should be 'sank'
				"weather",
				"Saturday", //actually should be 'on'
				"say","Sea","Sea","is","route","route","is","route","immigrants",
				"for","immigrants","trying","to","Union","Union","for","is","smugglers",
				"smugglers","take","take","take","take","coast","coast","from","coast",
				"islands","islands","to","islands","on","fragile","boats","on","take",
				"were","Two","of","those","were","be","reported","be","were"};

		
		String[] coveredTexts = new String[]{"Migrants","","","","","","","","drown","off",
				"Turkish","coast","At","least","40","migrants","drowned","in","the","Aegean",
				"Sea","off","the","western","coast","of","Turkey",",","officials","say",".",
				"The","corpses","found","washed","up","on","the","shore","near","the","city",
				"of","Izmir",".","Six","people","rescued",",","officials","told","Turkey","'",
				"s","Anatolia","news","agency",".","They","the","boat","was","overloaded",",",
				"carrying","more","than","60","people","when","it","sank","on","Saturday","in",
				"poor","weather",".","The","Aegean","Sea","a","major","route","for","illegal",
				"immigrants","trying","to","enter","the","European","Union",".","Correspondents",
				"say","smugglers","often","them","from","the","Turkish","coast","to","nearby",
				"Greek","islands","on","fragile",",","overcrowded","boats",".","Two","of",
				"those","rescued","reported","to","be","Palestinians","."};
		...
		while(iterator.hasNext()){
			...
			if(next.getHead()!=null){
				System.out.println(heads[head].equals(next.getHead().getCoveredText()) + ": " + next.getCoveredText() + "/" + heads[head]  + "/" + next.getHead().getCoveredText());
...
//	} // of initCas
	...
	public void testProcessWithNumTokensRestriction() throws IOException, InvalidXMLException, ResourceInitializationException, CASException,
	...
	/**
	 * First, checks if every token in the JCas has a non-empty depRel FSArray. 
	 * Second, checks if in every sentence there is only one token with a depRel.label 'null' or <no-type>. 
	 * Third, checks if in every sentence there is only one token with a depRel.head 'null'.
	 * 
	 * @param jcas
	...
	private boolean checkAnnotations(JCas jcas, Integer maxNumTokens) {
		...
		while (sentenceIterator.hasNext()) {
			...
			//only consider cases where no maxNumTokens has been defined or where sentence has less tokens than maxNumTokens
			if (maxNumTokens == null || tokenCounter <= maxNumTokens){
				...
				if (nullHeadCounter != 1) {
					LOGGER.error("There were " + nullHeadCounter + " head tokens in sentence " + sentence.getCoveredText());
					return false;
				}
				if (foundDepRelNull){
				...
				LOGGER.error("Found token.depRel in sentence that MST Parser should have skipped since it has > " + maxNumTokens + " tokens");
				return false;
			}
		} 
		return true;
