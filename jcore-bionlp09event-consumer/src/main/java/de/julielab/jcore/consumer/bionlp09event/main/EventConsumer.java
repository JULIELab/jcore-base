...
public class EventConsumer extends JCasAnnotator_ImplBase {
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		Type eventType = cas.getTypeSystem().getType(
				EventMention.class.getCanonicalName());
		try {
			if (!bioEventServiceMode) {
				...
				Iterator eventIterator = cas.getAnnotationIndex(eventType)
						.iterator();
				int i = 1;
				while (eventIterator.hasNext()) {
					EventMention eventMention = (EventMention) eventIterator
							.next();
				...
				EventWriter eventWriter = createEventWriter(documentText,
						a2File);
				Iterator eventIterator = cas.getAnnotationIndex(eventType)
						.iterator();
				int i = 1;
	...
	private EventWriter createEventWriter(String documentText, File file) {
		...
		try {
			...
			triggerWriter = new EventTriggerWriter(eventFileWriter,
					documentText);
		} catch (IOException e) {
		...
		return new EventWriter(eventFileWriter, null, triggerWriter, null);

	}

	private EventWriter createEventWriter(String documentText, String id)
		...
		try {
			proteinFileWriter = new BufferedWriter(new FileWriter(new File(
					directory, id + ".a1")));
			eventFileWriter = new BufferedWriter(new FileWriter(new File(
					directory, id + ".a2")));
			proteinWriter = new ProteinWriter(proteinFileWriter, documentText);
			triggerWriter = new EventTriggerWriter(eventFileWriter,
					documentText);
			entityWriter = new EntityWriter(eventFileWriter, documentText);
			return new EventWriter(eventFileWriter, proteinWriter,
					triggerWriter, entityWriter);
		} catch (IOException e) {
			throw new AnalysisEngineProcessException(e);
		}
	}
	...
	private DocumentWriter createDocumentWriter(String id) throws IOException {
		return new DocumentWriter(new BufferedWriter(new FileWriter(new File(
				directory, id + ".txt"))));
	}

	private String getDocumentId(CAS cas) {
		Header header = null;
		Type headerType = cas.getTypeSystem().getType(
