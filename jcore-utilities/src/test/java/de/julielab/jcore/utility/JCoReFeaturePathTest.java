...
public class JCoReFeaturePathTest {
	...
	public void testTypeInit() throws Exception {
		// Here, only the feature path parsing is tested. No actual feature values are retrieved from annotations.
		...
		// The path string should be broken down to single elements in an array. The leading slash should be removed
		...
		// Check whether the (recursive) extracted features are the correct ones.
		...
		assertArrayEquals(
				new Feature[] { geneType.getFeatureByBaseName("resourceEntryList"),
						resourceEntryType.getFeatureByBaseName("entryId") }, internalFeatures);

		// The first feature path element should have an index of 0, the second none, i.e. -1 to indicate that.
	...
	public void testGetValueAsString() throws Exception {
		// Here we test whether the correct value is retrieved via the feature path.
	...
	public void testGetValueAsStringOnValueArrayWithIndex() throws Exception {
		...
		// array and we provide a particular array index to retrieve the value from.
	...
	public void testGetValueAsStringOnSubtype() throws Exception {
		// Here we have the following case: Some annotation type - e.g. the ArgumentMention - defines a feature of
		// another annotation type, e.g. "Annotation" as it happens with the "ref" feature of the type
		// ArgumentMention. Now we happen to know that the actual type of "ref" in a particular use case - for example
		// Protein-Protein-Interaction (PPI) relations - is always "Gene", a far subtype of Annotation. Thus, in the
		// feature path we want to access the feature "specificType" of the Genes. The problem is now that the feature
		// type of "ref" in the type system is Annotation, thus there is no feature "specific" type. This case must be
	...
	/**
	 * 'Deep' means a recursive structure, a FeatureStructure containing another FeatureStructure.
	 * 
	 * @throws Exception
	 */
	@Test
