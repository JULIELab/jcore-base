...
public abstract class AbstractMapper {
	...
	DocumentClassification getClassification(final JCas jcas, final String xmlFileName,
		...
		if (classInfo.containsKey(mainClassification)) {
			if (classInfo.get(mainClassification).length != 1)
				throw new IllegalArgumentException(
						"More than 1 " + mainClassification + " classification in " + xmlFileName);
			final String mainClass = classInfo.get(mainClassification)[0];
			Class<? extends DocumentClassification> aClass = classification2class.get(mainClass);
			if (aClass == null) {
			...
			final DocumentClassification classification = constructor.newInstance(jcas);
			...
			if (classInfo.get(subClassification) != null) {
				if (classInfo.get(subClassification).length != 1)
					throw new IllegalArgumentException(
							"More than 1 " + subClassification + " classification in " + xmlFileName);
