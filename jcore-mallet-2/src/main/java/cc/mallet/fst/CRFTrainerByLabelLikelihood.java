...
/**
 * In the future this class may go away in favor of some default version of
...
public class CRFTrainerByLabelLikelihood extends TransducerTrainer implements TransducerTrainer.ByOptimization {
	...
	/*
	 * KT: 14.08.2008 the number of rounds of successive convergence used by the trainOptimized*
	 * methods
	 */
	...
	private int cachedValueWeightsStamp = -1; // ... re-calculate expectations and values to
												// getValue() because weights' values changed
	private int cachedGradientWeightsStamp = -1; // ... re-calculate to getValueGradient()
													// because weights' values changed
	private int cachedWeightsStructureStamp = -1; // ... re-allocate crf.weights, expectations &
													// constraints because new states, transitions
	// Use mcrf.trainingSet to see when we need to re-allocate crf.weights, expectations &
	// constraints because we are using a different TrainingList than last time
	...
	public CRFOptimizableByLabelLikelihood getOptimizableCRF(InstanceList trainingSet) {
		if (cachedWeightsStructureStamp != crf.weightsStructureChangeStamp) {
			...
			// reallocateSufficientStatistics(); // Not necessary here because it is done in the
			// constructor for OptimizableCRF
			ocrf = null;
	...
	// instance variables?
	// ANSWER: Yes and yes, but you have to use special syntax in the subclass ctor (see mallet-dev
	// archive) -cas
	 ...
	 * KT, 14.10.2008
	 * 
	 * "optimized" training where termination criterion is later that in "normal" train functions.
	 * Optimizer must converge n successive rounds. This avoid early stopping due to flip on gradient.
	 * 
	...
	public boolean trainOptimized(InstanceList training, int numIterationsPerProportion, double[] trainingProportions) {
		...
		for (int i = 0; i < trainingProportions.length; i++) {
			...
			if (trainingProportions[i] == 1.0)
				...
				converged = this.trainOptimized(training.split(new Random(1), new double[] { trainingProportions[i],
						1 - trainingProportions[i] })[0], numIterationsPerProportion);
	...
	/**
	 * Train a CRF on various-sized subsets of the data. This method is typically used to accelerate
	 * training by quickly getting to reasonable parameters on only a subset of the parameters
	 * first, then on progressively more data.
	 * 
	 * @param training
	 ...
	 *            If non-null, train on increasingly larger portions of the data, e.g. new double[]
	 *            {0.2, 0.5, 1.0}. This can sometimes speedup convergence. Be sure to end in 1.0 if
	 *            you want to train on all the data in the end.
	...
	public boolean train(InstanceList training, int numIterationsPerProportion, double[] trainingProportions) {
		...
		for (int i = 0; i < trainingProportions.length; i++) {
			...
			if (trainingProportions[i] == 1.0)
				...
				converged = this.train(training.split(new Random(1), new double[] { trainingProportions[i],
						1 - trainingProportions[i] })[0], numIterationsPerProportion);
			trainingIteration += numIterationsPerProportion;
		}
		return converged;
	...
	public boolean trainWithFeatureInduction(InstanceList trainingData, InstanceList validationData,
					...
					int numIterationsBetweenFeatureInductions, int numFeatureInductions,
					int numFeaturesPerFeatureInduction, double trueLabelProbThreshold,
	...
	/**
	 * Train a CRF using feature induction to generate conjunctions of features. Feature induction
	 * is run periodically during training. The features are added to improve performance on the
	 * mislabeled instances, with the specific scoring criterion given by the {@link FeatureInducer}
	 * specified by <code>gainName</code>
	 * 
	 * @param training
	 *            The training Instances.
	 * @param validation
	 ...
	 *            If the model's probability of the true Label of an Instance is less than this
	 *            value, it is added as an error instance to the {@link FeatureInducer}.
	 * @param clusteredFeatureInduction
	 *            If true, a separate {@link FeatureInducer} is constructed for each label pair.
	 *            This can avoid inducing a disproportionate number of features for a single label.
	 * @param trainingProportions
	 *            If non-null, train on increasingly larger portions of the data (e.g. [0.2, 0.5,
	 *            1.0]. This can sometimes speedup convergence.
	 * @param gainName
	 *            The type of {@link FeatureInducer} to use. One of "exp", "grad", or "info" for
	 *            {@link ExpGain}, {@link GradientGain}, or {@link InfoGain}.
	 * @return True if training has converged.
	 */
	public boolean trainWithFeatureInduction(InstanceList trainingData, InstanceList validationData,
					InstanceList testingData, TransducerEvaluator eval, int numIterations,
		...
		for (int featureInductionIteration = 0; featureInductionIteration < numFeatureInductions; featureInductionIteration++) {
			...
			if (trainingProportions != null && featureInductionIteration < trainingProportions.length) {
				logger.info("Training on " + trainingProportions[featureInductionIteration]
								+ "% of the data this round.");
				InstanceList[] sampledTrainingData = trainingData.split(new Random(1), new double[] {
			...
			InstanceList errorInstances = new InstanceList(trainingData.getDataAlphabet(), trainingData
							.getTargetAlphabet());
			...
			for (int i = 0; i < numLabels; i++)
				for (int j = 0; j < numLabels; j++) {
					clusteredErrorInstances[i][j] = new InstanceList(trainingData.getDataAlphabet(), trainingData
									.getTargetAlphabet());
				...
				for (int j = 0; j < trueOutput.size(); j++) {
					...
					if (trueLabelProb < trueLabelProbThreshold) {
						logger.info("Adding error: instance=" + i + " position=" + j + " prtrue=" + trueLabelProb
										+ (label == latticeLabeling.getBestLabel() ? "  " : " *") + " truelabel="
										+ label + " predlabel=" + latticeLabeling.getBestLabel() + " fv="
			...
			if (clusteredFeatureInduction) {
				...
				for (int i = 0; i < numLabels; i++) {
					for (int j = 0; j < numLabels; j++) {
						// Note that we may see some "impossible" transitions here (like O->I in a
						// OIB model)
						// because we are using lattice gammas to get the predicted label, not
						// Viterbi.
						...
						logger.info("Doing feature induction for " + crf.outputAlphabet.lookupObject(i) + " -> "
										...
										+ clusteredErrorInstances[i][j].size() + " instances");
						if (clusteredErrorInstances[i][j].size() < 20) {
							logger.info("..skipping because only " + clusteredErrorInstances[i][j].size()
											+ " instances.");
							continue;
						}
				...
				logger.info("CRF4 FeatureSelection now includes " + crf.globalFeatureSelection.cardinality()
								+ " features");
				klfi = null;
			}
	...
	/**
	 * Sets whether to use the 'some unsupported trick.' This trick is, if training a CRF where some
	 * training has been done and sparse weights are used, to add a few weights for feaures that do
	 * not occur in the tainig data.
	 * <p>
