...
//	HashSet<String> h;
	...
	public Alphabet(int capacity, Class entryClass) {
		...
		// someone could try to deserialize us into this image (e.g., by RMI).
		// Handle this.
	...
	public int lookupIndex(Object entry, boolean addIfNotPresent) {
		...
		if (map.containsKey(entry)) {
			...
			// Commented out because JNET sometimes threw this error but the
			// exact cause is unknown; it uses the empty label for the target
			// alphabet when predicting, perhaps this results in a clash somehow
			// (doen't really make sense because then it would be added once and
			// then everything would be OK, but the error happened just at some
			// random time in the middle of processing...)
			// if (h == null)
			// h = new HashSet<String>();
			// String name = Thread.currentThread().getName();
			// h.add(name);
			// if (h.size() > 1) {
			// System.err.println("Threads using Alphabet " +
			// System.identityHashCode(this));
			// for (String s : h)
			// System.err.println(s);
			// throw new
			// IllegalStateException("Alphabet used AND MODIFIED by multiple threads!");
			// }

			retIndex = entries.size();
			// System.out.println("Thread " + Thread.currentThread().getName() +
			// " has Alphabet: " + System.identityHashCode(this) + " with map: "
